...
//	     LOGFI();
		...
		RotatedRect box = fitEllipse(pointsf);//Fits an ellipse around a set of 2D points.
    	...
    	ellipse(cimage, box, Scalar(0,0,255), 1);//Draws a simple or thick elliptic arc or fills an ellipse sector.
    	ellipse(cimage, box.center, box.size*0.5f, box.angle, 0, 360, Scalar(0,255,255), 1);

		center[i].x= box.center.x;
		center[i].y= box.center.y;
		circle( cimage, box.center ,3 , CV_RGB(0,255,0),1, 8, 3 );//Draws a circle.
		
		LOGFI("pic:%d-%d width: %d height: %d",num,LR,(int)box.size.width,(int)box.size.height);

		if( MAX(box.size.width, box.size.height) > MIN(box.size.width, box.size.height)*30 )
			continue;
        
		if(box.size.width < 15)
			continue;
		if(box.size.width > 2*240)
			continue;

		if((int)box.center.y > 0 && (int)box.center.y < height && (int)box.center.x > 0 && (int)box.center.x < width && (width - (int)box.center.x)>0)	{
			int centerx=(width -  (int)box.center.x)/zoomx;
			int centery= (int)box.center.y/zoomy;
